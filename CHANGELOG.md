...
### 초기화 크롤링 릴리스: 이전 DB 업데이트 로직 비활성, 이전 DB 는 백업 뒤 비움(모바일·map 계약과 함께)

- 이번 업데이트는 초기화 크롤링을 한 번 무조건 하므로 이전 DB 를 새 구조로 옮기지 않는다. `upgrade_schema` 의 업데이트 로직(업그레이드 전 백업·열 추가·
  번호 붙은 마이그레이션·감시목록 열 이관·entry_value 재분류·synced_at 백필·상태 정규화)을 주석으로 남겨 껐다. 새 DB 는 지금 스키마로 만든다.
- 기존 DB 가 있으면(이전 버전 스키마): 서버 시작 때 `data/backups/legacy_v<옛 버전>_<시각>.db` 로 통째로 백업하고 무결성 검사 → 커뮤니티 데이터셋 선회전 →
  한 트랜잭션으로 신고 자료를 비우고 지금 스키마로 다시 만든다. 관리자 계정·API 키·감시목록·지오코딩 캐시는 남긴다(관리자·API 키 구조가 다르면 비우지 않고 멈춤).
  예전에 직접 고친 값·중복 판단·메모는 옮기지 않는다. 초기화 크롤링 화면·배너가 이 사실과 백업 경로를 안내한다.
- 처음 실행(신고 0건, 비운 기록 없음)은 초기화 크롤링 안내·크롤 409 없이 평소대로 — 공식 계정이 있으면 완료 기준선을 한 번 적는다.
- 이전(또는 더 새) 버전 DB 가져오기 거절: 서버 DB 복원은 스키마 4, 모바일 DB 복원은 앱 스키마 15 만 받는다(409, 바꾸기 전에 멈춤).
- GPT-6-Sol 검토 반영: 서버를 동시에 두 번 띄워도 두 번째가 이미 비운 DB 를 다시 비우지 않는다(쓰기 잠금 안에서 버전 재확인),
  크롤러 직접 실행(`--reset` 포함)은 이전 버전 DB 를 백업 없이 지우지 않고 멈춘다, `--reset` 이 비운 기록(`legacy_reset`)을 지우지 않는다.
  FTS 같은 가상 표가 있는 이전 DB 도 비운다(가상 표를 먼저 지워 보조 표 DROP 에서 멈추지 않게).
- `/settings/community/rebuild`·`/api/v1/community/rebuild` 응답에 `legacy_reset` 추가(없으면 null), 상태 `not_required` 추가.
- 테스트: `tests/test_storage_migration.py` 를 새 동작 기준으로 다시 씀(이전 DB 거절·백업 뒤 비우기·남길 표·되돌림·새 DB·업데이트 단계 미호출),
  복원 거절(서버 v0~3, 모바일 v0·1·10·14·16), 새 설치·비운 사용자 초기화 판정. 마이그레이션을 검사하던 테스트는 새 동작 검사로 바꿨다.
  대조 실험(수정을 되돌리면 해당 테스트 실패) 8건. 전체 427 OK(skip 4), 서버↔모바일 왕복 검사 차이 0.
  v2.5.3 코드로 만든 DB 로 fixture 서버를 띄워 백업·비우기·관리자 로그인·옛 API 키·재시작 뒤 재비우기 없음, 새 설치 `not_required` 를 확인했다.

## 2026-09-26 (dev, 미배포)

### 최근 7일 코드 감사(GPT-6-Sol) 반영 — 서버 쪽

- **SOL-02·03(모바일 레포와 함께)** 모바일 DB 복원이 모르는 열의 값을 조용히 버리지 않고 교체 전에 409 로 멈춘다(값이 모두 NULL 인 열은 통과).
  entry_value 는 앱 값이 원천: 빈 문자열도 덮어쓰고, NULL 은 서버 행 없음, 열이 없는 구앱이면 서버 값 유지.
- **SOL-04** 복원 중 동시 쓰기: 스테이징 복사부터 파일 교체까지 운영 DB 연결을 막는 장벽(`core/database/write_barrier.py`). 이미 빌린 연결은 반납을 기다리고(20초 넘으면 409),
  그동안 새 연결은 기다렸다가 새 파일로 다시 연다. 예전에는 복사와 교체 사이의 커밋(예: API 키 추가·편집)이 새 DB 에서 사라질 수 있었다.
  복원 검사 직후 크롤러가 시작되는 경쟁도 막았다(크롤 시작과 같은 잠금에서 '복원 중' 표시, 겹친 대기 큐는 되돌림).
- 크롤 중지는 크롤러가 실제로 끝난 것을 확인한 뒤에만 '중지됨' 상태가 된다(그 전에는 복원 거부). 대기 큐는 파일에도 남아 재시작 뒤 유지되고,
  저장에 실패하면 대기열에 넣었다고 답하지 않는다. 대기 큐 자동 시작은 한 번에 하나, 게이트·1회 초기화 검사 뒤 복원이 끼면 시작하지 않고,
  크롤러가 **번호별로 저장까지 끝냈다고 보고한 번호만** 큐에서 뺀다(로그인·조회·저장 실패한 번호는 남기고 1분→최대 30분 간격으로 다시 시도, 목록에 없는 번호는 제외).
  크롤 시작 경쟁에서 실행 중인 크롤의 로그를 지우지 않는다.
  대기 큐의 목록에 없는 번호는 목록 전체를 한 번 성공적으로 받은 뒤에만 '없음'으로 확정하고, 여러 신고에 걸리는 번호는 요청 때 거부하거나
  (목록을 받은 뒤 확인되면) 처리하지 못한 번호로 남겨 `/api/v1/crawl/status` 의 `unresolved`(추가 필드)로 보인다.
- **SOL-01(동등성)** 업로드 전에 ACK 받은 이벤트의 남은 대기 행을 정리(모바일과 같은 단계).
- 공유 자료 삭제 뒤 대기 사본 정리·업로드 연결 파일 문제가 함께 생기면 설정 카드가 둘 다 알린다.
- 테스트: `tests/test_storage_exchange.py`(계약 열 일치, 모르는 열, entry_value 세 상태, 복원 중 동시 쓰기 3건 — 장벽을 끄면 실패), 업로더 1건, 브라우저 스펙 1건.
  전체 380 OK(skip 4), 서버↔모바일 왕복 검사(`scripts/dev/db_roundtrip_check.py`) 차이 0.
...
- 서버 수정: 목록 반영이 빈 식별 값(상태·신고번호·신고명·신고일)으로 기존 값을 덮지 않음 · 통계에서 법규 필터로 카테고리가 비어도 법규 선택지를 유지.
- 계약: `change_tracked`(변경 판정 열 목록)를 계약 파일에 넣고 양쪽 테스트로 묶음, 모바일 스키마 15. 파서 공용 벡터 2건 추가(빈 주소 다음 후보, `C_NOW` 소수 표기).
- 결과: 왕복 검사·계산 동등성 모두 fixture·운영 사본(3,063건) 차이 0.

### 다크 모드 B안 "딥 다크" — 모바일과 같은 배색 (G16 W5)

- 예전 다크는 바탕 `#060e22`(채도 70%) 등 남색 계열이라 "블루 모드"처럼 보였다. 바탕·표면·테두리·사이드바를 채도 거의 없는 검정 계단으로 바꿨다(`docs/design/dark-palette.md`).
- 역할 분리: `--sr-primary`(다크 `#60a5fa`)는 글자·아이콘·장식, 새 `--sr-primary-fill`(다크 `#2563eb`)은 흰 글자를 올리는 채움(버튼·선택 표시). 예전엔 하나로 둘 다 해서 채운 버튼 흰 글자 대비가 3.1~3.7 이었다 → 5.17. 링크색도 토큰으로.
- 모바일 `SrColors.dark`·`AppTheme` 와 같은 값 — 공용 `contracts/dark-palette.json`(두 레포 바이트 동일, 왕복 검사 sha 비교), 테스트 `tests/test_dark_palette.py`(값 일치·대비·바탕 채도 10% 미만).
- 상태 배지색과 지도 위 팝업·마커(의도적으로 테마 무관)는 그대로.
...
- 구앱 호환: `/api/v1/crawl/start` 의 `login_mode` 는 무시, `/api/v1/crawl/resume` 은 410(구앱은 이 응답에 안내를 띄우지 않지만 크롤링은 회원으로 진행되므로 기능 손실 없음). 모바일도 같은 커밋 단위로 선택지를 없앴다.
- 테스트 `tests/test_crawl_modes_removed.py`.
...
- 차량번호: 같은 줄 안에서만 읽고 `*`·`(위` 에서 끊음. 칸이 비면 다음 줄을 번호로 가져가던 결함 수정(운영 사본 11건 해당). 기존 데이터는 업데이트 후 첫 기동 때 저장된 원문으로 다시 뽑음(`maintenance_service.repair_car_numbers`, 네트워크 없음).
- 발생일자 구분자 `.` `-` `/` 허용, 답변 처리상태가 '검토중'이면 다른 칸 사용(사용자 결정: 모바일 규칙), 처리내용의 전각 숫자·nbsp 정리(전각이면 금액을 못 읽고 '미확인'이 되던 결함), 값이 null 인 키는 없음으로(예전엔 멈추거나 'None').
- 상세 응답의 만족도 점수도 별점으로 저장(사용자 결정). 조회 실패면 사유는 그대로, 확정 미참여일 때만 '참여 가능'으로.
- 확인: 신고일은 서버·모바일 모두 날짜만 저장(운영 사본 3,063건 중 시각 포함 0건) — 합성 데이터만 시각이 붙어 있었다.

### 업데이트 뒤 한 번 훑기 + 화면 하단 진행 표시줄(서버)
...
- 검수: `docs/reviews/2026-09-25-gemini-g14-w1-review.md`.

### 웹 리뉴얼 확산 W3: 중복 신고 관리

- 상태 카드를 개수 달린 세그먼트 탭으로, 배지는 공통 알약형, 일괄 처리 줄은 작업 바 톤으로. 두 테마·휴대폰 폭 정돈.
- 그룹마다 "사용자 판단 · 마지막 저장 시각" 또는 "자동 감지" 표시(판단 이력 화면의 1차 — 판단 표는 마지막 판단만 보관).
- 테스트: `duplicates-manage.spec.ts`, 서비스 단위 테스트 추가.

### 웹 리뉴얼 확산 W3: 데이터 수정

- 분류 탭을 세그먼트로, 처리상태·"수정됨" 배지를 공통 알약형으로, 표 머리글·폼 색을 토큰으로(다크 테마에서 검은 머리글·회색 글자 제거).
- 세그먼트 탭·빈 목록 안내 스타일을 `theme.css` 공통으로 옮김(통계·중복 관리·데이터 수정이 함께 사용).
- 테마 시범 테스트가 CDN 이 느릴 때 목록 표를 5초만 기다리던 것을 초기화 대기 + 15초로. Playwright 120 통과(3개 브라우저).

### 웹 리뉴얼 확산 W4: 신고 지도(지도 바깥)

- 머리 영역의 그라데이션 배너를 간단한 안내 + 칩으로, 요약 카드·진행 카드·빈 화면·미변환 주소 창을 토큰으로, 연도("답변 연도")·분류 버튼을 세그먼트로.
- 지도 위 마커·툴팁·팝업은 밝은 지도 타일 위 데이터 색이라 그대로 둠. Playwright 132 통과(3개 브라우저).

### 웹 리뉴얼 확산 W2: 앱 설정 · 관리자 계정 · 기기 연동 · 백업 · 파일 브라우저

- 원색 카드 머리(파랑·초록·하늘색 바탕)를 기본 머리 + 강조 아이콘으로, `bg-white`·`table-light` 제거(다크 테마 흰 띠 없앰), 긴 설정 폼의 저장 버튼이 화면 아래에 따라옴.
- 기기 연동: 새 API 키 이름을 HTML 로 넣던 것을 글자로(이름에 태그를 넣으면 실행되던 문제).
- 테스트 `settings-pages.spec.ts`(두 테마 휴대폰 폭, 설정 저장 왕복, 키 생성·삭제, 백업 다운로드).

## 2026-09-24 (dev, 미배포)

### 웹 리뉴얼 P3(대시보드) 준비 · 테스트 안정화

- P3 대시보드 리뉴얼은 브랜치 `gemini/p3-dashboard` 에 구현·통합 수정까지 끝냄(시범 승인 뒤 dev 병합). 검수: `docs/reviews/2026-09-24-gemini-g13-p3-review.md`.
- 목록 AND/OR 검색 테스트가 webkit 에서 가끔 30초 멈추던 것(창 닫히는 중 재클릭)을 고치고, 결과 검사를 추가.
- 휴대폰 폭에서 상세 조건 창을 열면 메뉴 버튼이 창 제목을 가리던 것 수정(G12 캡처에서 발견).

### 웹 리뉴얼 P2·P2b: 통계 화면 상단·요약

상태: 완료(dev). main·VERSION·릴리즈 없음.

- 상단에 현재 조건 요약(답변 연도·처리기관·법규·날짜 등)과 조건 초기화, 교통위반 과태료 요약(확정 / 추정, 현재 조건 반영 — 사용자 결정 D-STAT-1).
- 연도("답변 연도")·분류·묶는 기준을 세그먼트 탭으로, 위반법규는 칩으로, 표 열 선택은 접는 패널(보이는 열 수 표시)로.
- 버그 수정(D-STAT-8): 통계 행을 눌러 목록으로 갈 때 기관·담당자 이름의 `& # +` 가 잘리던 것, "법규 없음" 드릴다운이 0건이던 것. 재현 테스트 먼저 작성.
- 상세 조건 폼의 빈 값 처리 스크립트가 페이지의 모든 form 에 걸리던 것을 그 폼으로 한정.
- P2b: 분류별 요약 카드(총 신고·답변 완료·처리 중·평균 처리기간+표본 수)와 월별 신고/답변 막대 차트. 모바일 통계 요약과 같은 서버 함수·같은 조건이라 값이 같다(테스트로 확인).
- Playwright 87 통과(3개 브라우저), unittest 98 통과.
...
- 목록: 주요/보조 동작을 나눈 작업 바, 알약형 상태 배지(라이트·다크), 날짜 열 잘림 해소, 빈 결과 안내(넓은 표에서도 보이게 고정), 휴대폰 폭 검색칸 정돈.
- 상세 모달: 제목 옆 상태 배지, 핵심 필드 2열(휴대폰 1열), 신고내용·보완 요청 카드.
- 목록 전용 스타일은 목록 페이지에서만 로드(다른 화면 영향 없음). Playwright 66 통과(3개 브라우저).

### 저장 계층 재설계 R6(서버): 이벤트 루프 막힘 해소, 정리

상태: 완료(dev). main·VERSION·릴리즈 없음.

- 무거운 라우터(복원·편집·중복 판단·통계·목록 등)가 이벤트 루프를 막지 않게 스레드풀에서 실행. 크롤링 시작은 잠금으로 한 줄로.
- 감시목록 제거가 ID 도 받음. 안 쓰는 저장 함수 정리, 오래 안 온 기기의 알림 읽은 위치 자동 정리. 시각 형식 규칙 문서화.
- Safari 에서 통계 담당자 표가 1280px 에서 넘치던 것 수정.
- 편집기로 고친 주소도 지도 좌표를 채움(원본 좌표는 그대로). 종결된 주정차 신고의 사진 촬영 시각도 크롤링 때 다시 시도(6개월 이내, 회당 30건).
- 테스트: unittest 96, Playwright 36(3개 브라우저) 통과.

### 저장 계층 재설계 R4·R5(서버): 편집 표시, 기기별 변경 알림

상태: 완료(dev). main·VERSION·릴리즈 없음.

- 데이터 수정 폼이 고친 칸에 "수정됨"·사이트 원본값·되돌리기를 보여 줌. 편집 API 응답에 `overrides`·`site_values` 추가(기존 필드 유지).
- `/api/v1/crawl/results` 가 기기별 읽은 위치를 씀(`device_id`) — 기기가 여럿이어도 모두 받음. 식별자 없는 구앱은 예전처럼 동작. 상태 JSON 원자적 쓰기.
- 모바일 짝 변경은 `feature/storage-refactor` 브랜치(에뮬레이터에서 편집 표시 확인).
- 테스트: unittest 92, Playwright 24 통과.

### 저장 계층 재설계 R2(서버): 신고 1건 = 트랜잭션 1개, 편집·판단 보존

상태: 완료(dev). main·VERSION·릴리즈 없음.

- 크롤링 저장을 `core/storage/reports_repo.py` 로 새로 작성: 트랜잭션 전에 네트워크, 1건씩 상세·본문·목록·화면용 표를 함께 커밋, 받는 즉시 저장(중간에 멈춰도 받은 만큼 남음), 실패 보고.
- 편집기는 수정값 표에 저장 → 재크롤링이 편집을 되돌리지 않음. 중복 판단은 판단 표가 원천 → 초기화 크롤링에도 유지. 별점 점수 기록.
- 화면용 표 규칙 하나(상세 + 수정값 + 감시목록 + 6개월 첨부 가림). 모바일 가져오기는 원본(title+detail)을 읽도록 함께 수정(왕복 검사가 잡은 원본 손실 경로).
- 재크롤링 대상에 종결여부 NULL 포함, 목록 저장의 만족도 되돌림 금지, 큐 신고번호 정확 일치, 인덱스(스키마 v4).
- 테스트: unittest 87 통과, 합성·운영 사본 왕복 차이 0.
...
- 통계 표(`d38b693`): 건수와 비율을 한 칸에(건수 아래 작은 비율) 합쳐 20열 → 13열. 확정/추정 과태료는 별도 열 유지. 위반법규 필터를 오른쪽 세로 패널에서 표 위 가로 줄로(`#statsLawSidebar` id 유지).
  1280·1440px 에서 기관별·담당자별 표가 영역 안에 들어옴. 합계 행 칸 수가 머리글보다 2개 적어 마지막 두 열(기타·미분류 비율, 별점) 합계가 비던 결함(열 분리 때 Opus 가 만든 것)도 수정.
  회귀 테스트 `tools/web-tests/specs/stats-layout.spec.ts`. 폰 폭에서는 여전히 가로 스크롤(P2 에서 다룸).
- 로고(`cea0752`, `f442759`): 사이드바·로그인·설정 화면은 LOGO 시안(docs/design/reference/13)의 라이트용/다크용 두 벌을 테마로 전환. 파비콘·아이폰 홈 아이콘·서버 실행 파일 아이콘(`mysafetyreport.ico`/`.icns`)은 카메라 앱 아이콘(14).
  기존 `logo.png` 2.4MB → 0.3MB. 모바일 앱 아이콘은 모바일 레포에서 처리.
- 다크 테마 잔여(`d434ee9`): 신고 지도 배너·숫자 카드·빈 상자, 중복 신고 대표건 행(`table-primary` 등 표 강조 행 전반), 설정의 구글 인증 파일·스케줄러 상자, 목록 선택 행·다중선택 드롭다운.
- 저장 계층 재설계: 계획 초안 `docs/plans/storage-refactor-plan.md`, Gemini 검수 기록 `docs/reviews/2026-09-24-gemini-g9-storage-review.md`. 코드 변경 없음.
