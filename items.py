...
def opendata_from_gc(engine, conn):
    ...
    try:
        ...
        print("opendata시트를 선택합니다.")
    except WorksheetNotFound:
        print("opendata시트가 확인되지 않습니다.")
    ...
    with engine.connect() as conn:
        ...
        print("정보공개청구 결과 입력 중")
        conn.commit()
        print("정보공개청구 결과 입력 성공")
...
def title_to_sql(dataframes, engine, conn):
    ...
    with engine.connect() as conn:
        for df in dataframes:
            ...
            i += 1
            print(f"{i}건 임시 리스트 테이블 밀어넣기 완료")
    print(f"총 {i}건 데이터 밀어넣기 완료")
...
def deatil_to_sql(dataframes, engine, conn):    
    ...
    with engine.connect() as conn:
        for df in dataframes:
            ...
            i += 1
            print(f"{i}건 개별 신고 결과 임시 테이블 밀어넣기 완료")
    print(f"총 {i}건 데이터 밀어넣기 완료")
...
def save_results(df):
    ...
    print(f"데이터 엑셀 저장 성공, 저장경로 : {os.path.join(settings.path, settings.resultfile)}")

    try:
        worksheet = spreadsheet.worksheet("data")
        print("data시트를 선택합니다.")
    except WorksheetNotFound:
        print("data시트가 확인되지 않습니다.")
        worksheet = spreadsheet.add_worksheet(title="data", rows="1000", cols="20")
        print("data시트를 생성합니다.")
    
    worksheet.clear()
    print("기존 구글 스프레드시트 데이터를 삭제합니다.")
    ...
    print("구글 스프레드시트에 새로운 데이터를 성공적으로 입력하였습니다.")