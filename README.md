...
- **상태 계층 정규화**: 원본 진행상황은 `상태`로 보존하고, 검색·통계·알림용 `처리상태`는 `처리중/보완요청/수용...` canonical 값으로 저장
...
- `entry_value`, `raw_content`, `synced_at`, 중복군 메타, watchlist, `last_sync` 와 함께 raw `상태` / canonical `처리상태` / 보완 컬럼도 함께 보존
