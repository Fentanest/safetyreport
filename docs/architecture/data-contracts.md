...
- **2026-09-26 감사 보강(SOL-02·03·04, 모바일 레포와 함께 변경)**:
  ...
  - 복원 중 동시 쓰기: 스테이징 복사부터 파일 교체까지 `core/database/write_barrier.exclusive()` 가 운영 DB 연결을 막는다.
    ...
    큐 지정 직접 시작도 실행마다 고유 큐 파일(`<queue|mobile_queue|web_selected_queue>_<uuid>.txt`)을 쓰고(R9-01), 끝나면 결과 보고를 읽어
    없음·모호 번호는 기록하고 처리하지 못한 번호(보고 없음·실패·목록 일부만 받음)는 대기 큐로 넘겨 다시 시도한다(R9-02·R9-04). 기록을 저장하지
    못한 없음·모호 번호도 대기 큐로 넘겨 다음에 다시 판정한다(R8-03). 서버 기동 때 지난 실행이 정리하지 못한 큐·보고 파일을 회수한다.
